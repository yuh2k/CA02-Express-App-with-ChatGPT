...
class nim_game1:
    def __init__(self):
        self.states = {"a": 10, "b": 10, "c": 10} # hashmap for states, default {"a": 10, "b": 10, "c": 10} 
        self.mapping = ["a", "b", "c"]    # Mapping relations between indices and letter "a", "b", "c"
...
class nim_game2:
    ...
    def computer(self):
        import random
        cnt = 0
        remove_index = random.randint(0, 2)   # generate a random remove index
        while self.states[self.mapping[remove_index]] == 0:  # if the number at remove index is 0, try next one
            remove_index = random.randint(0, 2)
        remove_val = random.randint(1, self.states[self.mapping[remove_index]])   # generate the remove value
        print("computer move:", self.mapping[remove_index], remove_val)
        print("removing", remove_val, "from", self.mapping[remove_index], "gives")
        self.states[self.mapping[remove_index]] -= remove_val
    ...
    def console(self):
        print("Let's play NIM!")
        print("NIM_State",self.states)
        message = ""  # default print final message
        while not self.check():
            print("")
            self.user()
            print("NIM_State",self.states)
            print("")
            if self.check():  # if the user remove the last number, the player loses. Then cease the game and print lose message
                message = "You lose! Computer wins!"
                break
            self.computer()
            print("NIM_State",self.states)
    ...
    def check(self):
        # Check if there is only one number that is not 0
        return set(self.states.values()) == {0}
